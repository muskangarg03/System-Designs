...
Not all tasks need to be completed immediately or require a response.
...
Client Application (Frontend)
   ...
   _____|_____
  |           |
Server 1   Server 2  (Application Code)
  |           |
  |___________|
        |
        v
  ...
  Application Code  --->  Message Queue  --->  SMS / Email Service

  All Components  --->  Monitoring & Logs
```
...
| Application Code | Business logic, API endpoints, database communication |
| Client Application | User-facing interface |
...
| Monitoring & Logs | Tracks system health, errors, and performance |

---

## Key Takeaways

- Every system revolves around **data** — storing it, processing it, and serving it efficiently.
- **APIs** abstract database complexity and provide clean endpoints for data operations.
- **Cache** reduces redundant database calls and improves speed.
- **Load Balancers** ensure no single server is overwhelmed.
- **Message Queues** decouple services and enable reliable asynchronous processing.
- **Monitoring** is essential to maintain system reliability, especially as new features are added.
